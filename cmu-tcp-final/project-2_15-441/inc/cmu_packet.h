...
typedef struct {
  ...
  uint16_t plen;               // Packet length. 包长度
  uint8_t flags;               // Flags.
 ...
 * 创建并初始化一个块
 *
 * @param src The source port. 源端口
 * @param dst The destination port. 目标端口
 * @param seq The sequence number. 序列号
 * @param ack The acknowledgement number. ack
 * @param hlen The header length. 头部长度
 * @param plen The packet length. 块长度
 * @param flags The flags. 标志位
 * @param adv_window The advertised window.
 * @param ext_len The header extension length.
 * @param ext_data The header extension data. 
 * @param payload The payload. 载荷
 * @param payload_len The length of the payload. 载荷长度
 *
 * @return A pointer to the newly allocated packet. User must `free` after use. 返回指针 需要 free
 */
